...
class DataGenerator(Sequence):
    ...
    def __init__(self, dataset, config, shuffle=True, augment=False, augmentation=None,
        ...
        self.temp_indexes = np.arange(len(self.image_ids))
    ...
    def __len__(self):
        if hasattr(self, 'config') and hasattr(self.config, 'IMAGES_PER_GPU'):
            return int(len(self.image_ids) / self.config.IMAGES_PER_GPU)
        else:
            return int(len(self.image_ids))

    def __getitem__(self, idx):

        temp_batch_idxs = [idx * self.batch_size +
...
class MaskRCNN():
    ...
    def find_last(self):
        ...
        # Pick last directory
        for dir_name in dir_names.reverse():
            dir_name = os.path.join(self.log_dir, dir_name)
            # Find the last checkpoint
            checkpoints = next(os.walk(dir_name))[2]
            checkpoints = filter(
                lambda f: f.startswith("mask_rcnn"), checkpoints)
            checkpoints = sorted(checkpoints)
            if not checkpoints:
                print("Could not find weight files in {}".format(dir_name))
                continue

            checkpoint = os.path.join(dir_name, checkpoints[-1])
            print('Found most recent checkpoint in ', checkpoint)
            return checkpoint
        raise FileNotFoundError
    ...
    def set_trainable(self, layer_regex, keras_model=None, indent=0, verbose=1):
        ...
        for layer in layers:
            ...
            if layer.name == 'conv1' and hasattr(self.config, 'depth_mode') and self.config.depth_mode:
                trainable = True
                print(
                    'Set conv1 layer to trainable, because training in depth mode requires so')
    ...
    def set_log_dir(self, model_path=None):
        ...
        if model_path:
            ...
            _, dirname = os.path.split(os.path.dirname(model_path))
            ...
            try:
                ...
                self.epoch = 0
            print('Re-starting from epoch %d' %
                  self.epoch, ' in directory ', dirname)

        # Directory for training logs
        self.log_dir = os.path.join(self.model_dir, dirname)
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers, args,
        ...
        # Both augmentation params default to 0
        if augmentation is not None:
            augm_num = args.augm_num
            augm_strength = args.augm_strength
        else: 
            augm_num = 0
            augm_strength = 0
        ...
        callbacks = [
            LRTensorBoard(log_dir=self.log_dir,
                          ...
                          augm_num=augm_num, augm_strength=augm_strength),
            keras.callbacks.ModelCheckpoint(self.checkpoint_path,
                                            verbose=0, save_weights_only=True, save_best_only=True,
                                            mode='min'),
            CSVLogger(os.path.join(self.log_dir, 'logs.csv'),
